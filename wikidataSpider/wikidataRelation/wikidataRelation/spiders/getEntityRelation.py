...
class entityRelationSpider(scrapy.spiders.Spider):
	...
	def parse(self, response):
		...
		#获取已经爬取的数据(避免重复爬)
		alreadyGet = []
		if(os.path.exists(os.path.join(filePath,"wheat_entity1_entity2.json"))):
			#读取文件
			with open(os.path.join(filePath,"wheat_entity1_entity2.json"),'r') as fr:
				for line in fr:
		...
		count = 0
		# /home/admin/Downloads/Agriculture_KnowledgeGraph-master/wikidataSpider/wikidataRelation/entities_processed.json
		with open(filePath+"/wikidataRelation/entities_processed.json","r") as fr:
			for line in fr.readlines():
				count += 1 
