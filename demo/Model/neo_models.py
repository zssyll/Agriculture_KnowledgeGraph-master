...
class Neo4j():
    ...
    def connectDB(self):
        self.graph = Graph(constant_url.NEO4J_URL, username=constant_url.NEO4J_USERNAME,
                           password=constant_url.NEO4J_PASSWORD)
    ...
    # 根据title值返回互动百科item
    def matchNodebyTitle(self, value):
        answer = None
    ...
    def getEntityRelationbyEntity(self, value):
        ...
        if answer == None:
            answer = self.graph.run("MATCH (entity1) - [rel] -> (entity2)  WHERE entity2.title = \"" + str(
            value) + "\" RETURN rel,entity1").data()
        return answer
    ...
    def findRelationByEntities(self,entity1,entity2):
        answer = self.graph.run("MATCH (p1{title:\"" + str(entity1) + "\"}),(p2{title:\""+str(entity2)+"\"}),p=shortestpath((p1)-[*..10]-(p2)) RETURN p").evaluate()
